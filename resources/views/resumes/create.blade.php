            ...
            <div class="glass dark:glass sm:rounded-3xl p-8 shadow-2xl">
                ...
                <form action="{{ route('resumes.store') }}" method="POST" class="space-y-8" x-data="{
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title') }}" required class="glass-input w-full" placeholder="e.g. Full Stack Developer">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Full Name</label>
                                <input type="text" name="full_name" value="{{ old('full_name') }}" class="glass-input w-full" placeholder="John Doe">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Email</label>
                                <input type="email" name="email" value="{{ old('email') }}" class="glass-input w-full" placeholder="john@example.com">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Phone</label>
                                <input type="text" name="phone" value="{{ old('phone') }}" class="glass-input w-full" placeholder="+1 234 567 8900">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="address" value="{{ old('address') }}" class="glass-input w-full" placeholder="123 Main St, City, Country">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Professional Summary</label>
                        <textarea name="summary" rows="4" class="glass-input w-full" placeholder="A brief summary of your professional background...">{{ old('summary') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <template x-for="(skill, index) in skills" :key="index">
                            <div class="flex items-center mb-3 group">
                                ...
                                <button type="button" @click="removeSkill(index)" x-show="skills.length > 1" class="ml-3 text-red-500 hover:text-red-700 bg-red-100 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
