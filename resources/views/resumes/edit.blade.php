    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="glass dark:glass sm:rounded-3xl p-8 shadow-2xl">
                ...
                <form action="{{ route('resumes.update', $resume) }}" method="POST" class="space-y-8" x-data="{
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title', $resume->title) }}" required class="glass-input w-full" placeholder="e.g. Full Stack Developer">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Full Name</label>
                                <input type="text" name="full_name" value="{{ old('full_name', $resume->full_name) }}" class="glass-input w-full" placeholder="John Doe">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Email</label>
                                <input type="email" name="email" value="{{ old('email', $resume->email) }}" class="glass-input w-full" placeholder="john@example.com">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Phone</label>
                                <input type="text" name="phone" value="{{ old('phone', $resume->phone) }}" class="glass-input w-full" placeholder="+1 234 567 8900">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="address" value="{{ old('address', $resume->address) }}" class="glass-input w-full" placeholder="123 Main St, City, Country">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Professional Summary</label>
                        <textarea name="summary" rows="4" class="glass-input w-full" placeholder="A brief summary of your professional background...">{{ old('summary', $resume->summary) }}</textarea>
                    </div>
                    ...
                    <div class="flex justify-end pt-6 border-t border-gray-200 dark:border-gray-700">
                        <x-primary-button class="px-8 py-3 bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 transform hover:scale-105 transition-all text-lg shadow-lg">
                            {{ __('Update Resume') }}
                        </x-primary-button>
                    </div>
                </form>
